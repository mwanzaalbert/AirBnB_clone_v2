...
class HBNBCommand(cmd.Cmd):
    ...
    def do_update(self, args):
        ...
        if '{' in args[2] and '}' in args[2] and isinstance(eval(args[2]), dict):
            ...
            args = args[2].split()
        if not args:  # ensure attributes are present
            print("** attribute name missing **")
            return
        if len(args) % 2 != 0:  # ensure even count of attr/values
            print("** value missing **")
            return
        ...
        for i in range(0, len(args), 2):
            ...
            storage.all()[key].__dict__.update({att_name: att_val})
