...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_fs_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User name="Johndoe" age=28 height=5.7')
            mdl_id = cout.getvalue().strip()
            self.assertIn('User.{}'.format(mdl_id), storage.all().keys())
            clear_stream(cout)
            cons.onecmd('show User {}'.format(mdl_id))
            self.assertIn("'name': 'Johndoe'", cout.getvalue().strip())
            self.assertIn("'age': 28", cout.getvalue().strip())
    ...
    def test_db_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            clear_stream(cout)
            cons.onecmd('create User email="admin2024@hbtn.com"' +
                        ' password="123"')
            mdl_id = cout.getvalue().strip()
            dbc = MySQLdb.connect(
    ...
    def test_db_show(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            self.assertIn('admin2024@hbtn.com', result)
            self.assertIn('1234', result)
            self.assertIn('admin2024@hbtn.com', cout.getvalue())
            self.assertIn('1234', cout.getvalue())
            cursor.close()
            dbc.close()
